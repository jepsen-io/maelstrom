...
/// Simplest thread-safe job queue that make threads wait in get().
...
pub fn WaitJobQueue(comptime T: type) type {
    return struct {
        head: ?*Node,
        tail: ?*Node,
        ...
        pub fn get(self: *Self) ?*Node {
            return self.get_internal(true);
        }

        /// Gets a previously inserted node or returns `null` if there is none.
        /// It is safe to `get()` a node from the queue while another thread tries
        /// to `remove()` the same node at the same time.
        pub fn try_get(self: *Self) ?*Node {
            return self.get_internal(false);
        }
 
        /// Gets a previously inserted node or returns `null` if there is none.
        /// It is safe to `get()` a node from the queue while another thread tries
        /// to `remove()` the same node at the same time.
        fn get_internal(self: *Self, wait: bool) ?*Node {
            self.mutex.lock();
            defer self.mutex.unlock();

            // allow to drain queue first before shutting down.
            if (wait) {
                while (self.head orelse null == null) {
                    if (self.shutdown_f) return null;
...
test "std.atomic.Queue" {
    ...
    var queue = WaitJobQueue(i32).init();
    var context = Context{
        .allocator = a,
        .queue = &queue,
        .put_sum = 0,
...
fn startPuts(ctx: *Context) u8 {
    ...
    while (put_count != 0) : (put_count -= 1) {
        ...
        const node = ctx.allocator.create(WaitJobQueue(i32).Node) catch unreachable;
        node.* = .{
            .prev = undefined,
            .next = undefined,
            .data = x,
...
test "std.atomic.Queue single-threaded" {
    ...
    var node_0 = WaitJobQueue(i32).Node{
        .data = 0,
        .next = undefined,
        .prev = undefined,
    };
    ...
    var node_1 = WaitJobQueue(i32).Node{
        .data = 1,
        .next = undefined,
        .prev = undefined,
    };
    ...
    var node_2 = WaitJobQueue(i32).Node{
        .data = 2,
        .next = undefined,
        .prev = undefined,
    };
    ...
    var node_3 = WaitJobQueue(i32).Node{
        .data = 3,
        .next = undefined,
        .prev = undefined,
    };
    ...
    var node_4 = WaitJobQueue(i32).Node{
        .data = 4,
        .next = undefined,
        .prev = undefined,
    };
